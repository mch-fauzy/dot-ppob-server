...
class MembershipService {
    ...
    static login = async (req) => {
        try {
            ...
            if (totalUsers === BigInt(0))
                throw failure_1.Failure.unauthorized('Email or Password is not valid');
            const user = users[0];
    ...
    static getByEmail = async (req) => {
        try {
            const membershipKey = `${constant_1.CONSTANT.REDIS.MEMBERSHIP_KEY}:${req.email}`;
            const membershipCache = await redis_cache_1.RedisUtils.getCacheByKey(membershipKey);
            if (membershipCache) {
                const response = JSON.parse(membershipCache);
            ...
            await redis_cache_1.RedisUtils.storeCacheWithExpiry(membershipKey, constant_1.CONSTANT.REDIS.CACHE_EXPIRY, JSON.stringify(response));
            return response;
        }
        catch (error) {
    ...
    static updateProfileByEmail = async (req) => {
        try {
            ...
            await redis_cache_1.RedisUtils.deleteCacheByKey(`${constant_1.CONSTANT.REDIS.MEMBERSHIP_KEY}:${req.email}`);
            return 'Success';
        }
        catch (error) {
